...
from .modeling_block import JointTransformerBlock
from .modeling_embedding import CombinedTimestepConditionEmbeddings, PatchEmbed3D, rope
...
class EmbedNDRoPE(nn.Module):
    ...
    def forward(self, ids: torch.FloatTensor) -> torch.FloatTensor:
        n_axes = ids.shape[-1]
        emb = torch.cat(
...
class PyramidMMDiT(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        if interp_condition_pos:
            print("------------------------------------------------------------------------------------")
            print("| Interpolation of the Positional Embeddings for the Condition Latents is enabled! |")
            print("------------------------------------------------------------------------------------")
        # fmt: on
    ...
    @torch.no_grad()
    def _prepare_pyramid_temporal_rope_ids(
        self, sample: List[torch.FloatTensor], batch_size: int, device: torch.device
    ...
    def merge_input(
        self,
        sample: List[torch.FloatTensor],
        encoder_hidden_length: List[int],
        ...
        if self.pos_embed_type == "rope":
            ...
            if self.temp_pos_embed_type == "rope" and self.add_temp_pos_embed:
                ...
                text_ids = torch.zeros(pad_batch_size, encoder_attention_mask.shape[1], 1).to(
                    device=device
                )
                input_ids_list = [
        ...
        for i_p in range(len(hidden_length)):
            ...
            stage_attention_mask = rearrange(token_ids, "b i -> b 1 i 1") == rearrange(
                token_ids, "b j -> b 1 1 j"
            )  # [bs, 1, q_len, k_len]
            if self.use_temporal_causal:
                input_order_ids = input_ids_list[i_p].squeeze(2)
                temporal_causal_mask = rearrange(input_order_ids, "b i -> b 1 i 1") >= rearrange(
                    input_order_ids, "b j -> b 1 1 j"
                )
    ...
    def split_output(
        self,
        batch_hidden_states: torch.FloatTensor,
        hidden_length: List[int],
        temps: List[int],
        heights: List[int],
        widths: List[int],
        trainable_token_list: List[int],
    ) -> List[torch.FloatTensor]:
    ...
    def forward(
        self,
        sample: List[List[torch.FloatTensor]],  # [num_stages]
        encoder_hidden_states: torch.FloatTensor,
        encoder_attention_mask: torch.FloatTensor,
        pooled_projections: torch.FloatTensor,
        timestep_ratio: torch.FloatTensor,
    ) -> List[torch.FloatTensor]:
        # Get the timestep embedding
        temb = self.time_text_embed(timestep_ratio, pooled_projections)

        # Get the input sequence (Merging different resolution inputs into a single token sequence)
        ...
        # Block by block application of the transformer attention
        for block_idx, block in enumerate(self.transformer_blocks):
            # Apply gradient checkpointing to later layers based on configured ratio
            should_checkpoint = (
                self.training
                and self.gradient_checkpointing
                and block_idx
                >= int(len(self.transformer_blocks) * self.gradient_checkpointing_ratio)
            )

            if should_checkpoint:
                # Checkpointing version (Makes it more memory friendly during training)
